...
export class ReviewPaymentPage extends BasePage {
  ...
  async enterCardNumber(cardNumber: string = process.env.CARD_NUMBER as string) {
    await this.cardNumberInput.fill(cardNumber);
  }

  async enterExpiryDate(expiryDate: string = process.env.CARD_EXPIRATION_DATE as string) {
    await this.expiryDateInput.fill(expiryDate);
  }

  async enterCVC(cvc: string = process.env.CARD_SECURITY_CODE as string) {
    await this.cvcInput.fill(cvc);
  }

  async enterZipCode(zipCode: string = process.env.ZIP_CODE as string) {
